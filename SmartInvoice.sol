...
contract SmartInvoice is Ownable {
    ...
    function initializeDummy() public onlyOwner {
        ...
        _invoice3.supplier = 0x1605906e66a25EEc5ef206a1C8e2f99F24E6E253;
        _invoice3.currentBuyer = 0x18D54cca8608d90661244AF0FEb0A3D3Ad367aCD;
        _invoice3.totalValue = 5000000;
        _invoice3.currentRiskRating = 3;
        _invoice3.dueDate = 0;

        invoiceList.push(_invoice3);

        invoiceSuppliers.push(0x1605906e66a25EEc5ef206a1C8e2f99F24E6E253);
    }

    function getInvoiceLength() public view returns (uint) {
        return invoiceList.length;
    }

    function getInvoice(uint invoiceId) public view returns (address,address,uint,uint,uint,bool,uint) {
    ...
    function getInvoiceRiskRating(uint invoiceId) public view returns(uint,uint) {
    ...
    function addInvoice(
       ...
       emit AddedInvoice(msg.sender, invoiceCount, now,_inn.supplier,_currentBuyer);
    }

    function applyRiskRating(uint invoiceId, uint rating) public onlyOwner {
        require(invoiceId != 0);
        Invoice memory _invoice = invoiceList[invoiceId];
        require (_invoice.supplier != address(0x0));
    ...
    function sellInvoice(uint invoiceId, address newBuyer) public {
        require(invoiceId != 0);
        require(newBuyer != address(0x0));
        Invoice memory _invoice = invoiceList[invoiceId];
        require (_invoice.supplier != address(0x0));
    ...
    function invalidateInvoice(uint invoiceId) public {
        require(invoiceId != 0);
        Invoice memory _invoice = invoiceList[invoiceId];
        require (_invoice.supplier != address(0x0));
    ...
    function finishInvoice(uint invoiceId) public {
        require(invoiceId != 0);
        Invoice memory _invoice = invoiceList[invoiceId];
        require (_invoice.supplier != address(0x0));
        require (_invoice.supplier == msg.sender);
